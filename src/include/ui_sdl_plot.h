...
/**
 * @brief plot_graph call: draw_plot function, when exit event received clean everything
 * @param params plot parameters (cf plot_params struct)
 */
int plot_graph(plot_params *params);

/**
 * @brief draw_scale_graduation: draw graduation for the graph
 * @param renderer SDL renderer object
 * @param params plot parameters (cf plot_params struct)
 * @param plot structure containing textures and surfaces
 * @param plot_width plot base width (with proportion to screen width)
 * @param plot_heigth plot bas heigth (with proportion to screen heigth)
 * @param plot_mask_position SDL rectangle giving position of plot base (x,y) from max x or max y due to stroke width
 * @param font SDL font used for captions
 * @param font_color font color to be used
 * @param surface_list list of surfaces stored to be freed later
 * @param plot_position_x x position of plot
 * @param plot_position_y y position of plot
 */
...
/**
 * @brief draw_points: draw each coordinate points on the graph featuring cartesian coordinate system
 * @param renderers SDL renderer
 * @param caption_item caption item structure
 * @param params plot parameters (cf plot_params struct)
 * @param plot_width plot base width (with proportion to screen width)
 * @param plot_heigth plot bas heigth (with proportion to screen heigth)
 * @param plot_mask_position SDL rectangle giving position of plot base (x,y) from max x or max y due to stroke width
...
/**
 * @brief draw_plot: create a new SDL window and plot grap with given parameters
 * @param splot plot containing SDL objects
 * @param params plot parameters (cf plot_params struct)
 * @param surface_list list of surfaces stored to be freed later
 */
...
/**
 * @brief wait_for_sdl_event: wait for an event
 */
void wait_for_sdl_event(void);

/**
 * @brief clean_plot: full clean of SDL pointers and linked list clear
 * @param splot plot containing SDL items
 * @param params plot parameters (cf plot_params struct)
  * @param surface_list list of surfaces stored to be freed later
 */
