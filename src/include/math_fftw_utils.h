...
void fill_random_1d_cplx_one(int n, fftw_complex *arr);
...
int compare_2d_cplx(int rows, int cols, fftw_complex *ref, fftw_complex *arr, double eps);
...
void fill_random_1d_cplx_one(int n, fftw_complex *arr) {
    double real, imag;
    for (int i = 0; i < n; ++i) {
        real = ((i >= (40 + n/2)) & (i <= (60 + n/2))) ? 1.0 : 0.0;
        imag = 0.0;
        arr[i] = real + I * imag;
    }
}
