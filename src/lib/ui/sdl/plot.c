 ...
 * @param renderers SDL renderer object
 * @param caption caption item object structure
 ...
 */
void draw_points(SDL_Renderer *renderer, caption_item *caption, plot_params *params, float plot_width, float plot_heigth, SDL_Rect plot_mask_position)
{
	...
	while (tmp!=NULL)
	{
		if (tmp->caption_id==caption->caption_id)
		{
			...
			SDL_SetRenderDrawColor(renderer,(caption->caption_color & 0xFF0000)>>16, 
				(caption->caption_color & 0x00FF00)>>8,
				caption->caption_color & 0x0000FF,255);
